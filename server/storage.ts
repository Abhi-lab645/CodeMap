...
export class DbStorage implements IStorage {
  ...
  // Big Project methods
  async getAllBigProjects(): Promise<BigProject[]> {
    return await db.select().from(bigProjects);
  }

  async getBigProject(id: string): Promise<BigProject | undefined> {
    const result = await db.select().from(bigProjects).where(eq(bigProjects.id, id));
    return result[0];
  }

  async createBigProject(insertProject: InsertBigProject): Promise<BigProject> {
    const result = await db.insert(bigProjects).values(insertProject).returning();
    return result[0];
  }
  ...
  async getMiniProjectsByBigProjectId(bigProjectId: string): Promise<MiniProject[]> {
    return await db
      .select()
      .from(miniProjects)
      .where(eq(miniProjects.bigProjectId, bigProjectId))
      .orderBy(miniProjects.miniId);
  }

  async getMiniProject(id: string): Promise<MiniProject | undefined> {
    const result = await db.select().from(miniProjects).where(eq(miniProjects.id, id));
    return result[0];
  }

  async createMiniProject(insertMiniProject: InsertMiniProject): Promise<MiniProject> {
    const result = await db.insert(miniProjects).values(insertMiniProject).returning();
    return result[0];
  }
    ...
    const result = await db
      ...
      .where(
        and(
          ...
          eq(userProgressTable.bigProjectId, bigProjectId)
        )
      );
    return result[0];
  }
  ...
  async createUserProgress(insertProgress: InsertUserProgress): Promise<UserProgress> {
    const result = await db.insert(userProgressTable).values(insertProgress).returning();
    return result[0];
  }
  ...
  async createUserBadge(insertUserBadge: InsertUserBadge): Promise<UserBadge> {
    const result = await db.insert(userBadgesTable).values(insertUserBadge).returning();
    return result[0];
  }
}
...
async function seedDatabase() {
  ...
  const existingProjects = await dbStorage.getAllBigProjects();
  ...
  // Create sample big projects
  const pythonProject = await dbStorage.createBigProject({
    title: "E-Commerce Platform",
    description: "Build a complete e-commerce website with product listings, shopping cart, and payment integration",
    language: "Python",
    difficulty: "Intermediate",
  ...
  const jsProject = await dbStorage.createBigProject({
    title: "Social Media Dashboard",
    description: "Create a social media analytics dashboard with real-time data visualization",
    language: "JavaScript",
    difficulty: "Advanced",
  ...
  const reactProject = await dbStorage.createBigProject({
    title: "Task Management App",
    description: "Develop a modern task management application with drag-and-drop, notifications, and team collaboration",
    language: "React",
    difficulty: "Beginner",
  ...
  // Helper functions
  const getMiniProjectTitle = (id: number): string => {
    const titles = [
      "Setup & Variables",
      "Data Structures",
      "Control Flow",
  ...
  const getMiniProjectConcepts = (id: number): string[] => {
    const concepts = [
      ["Variables", "Data Types", "Input/Output"],
      ["Lists", "Dictionaries", "Arrays"],
      ["If/Else", "Loops", "Conditionals"],
  ...
  const getMiniProjectTheory = (id: number): string => {
    return `In this mini project, you'll learn about ${getMiniProjectConcepts(id)[0]}.\n\nThis is a fundamental concept in programming that allows you to ${
      id === 1
        ? "store and manipulate data"
        : id === 2
        ? "organize data efficiently"
  ...
  const getMiniProjectExamples = (id: number, language: string): string[] => {
    if (language === "python") {
      return [
        `# Example ${id}.1\nx = 10\ny = 20\nprint(x + y)`,
        `# Example ${id}.2\ndef greet(name):\n    return f"Hello, {name}!"\n\nprint(greet("World"))`,
  ...
  const getMiniProjectTask = (): string => {
    return `Complete the following task:\n\n1. Implement the required functionality\n2. Test your code thoroughly\n3. Ensure it handles edge cases\n4. Follow best practices for code organization\n\nYour solution should demonstrate understanding of the concepts covered in the theory section.`;
  };

  const getMiniProjectHint = (): string => {
    return `Hint: Start by breaking down the problem into smaller steps. Remember the examples you saw in the theory section - they contain patterns you can use here!`;
  };

  const getCodeTemplate = (id: number, language: string): string => {
    if (language === "python") {
      return `# Your code here\n# Complete the task described above\n\ndef main():\n    # TODO: Implement your solution\n    pass\n\nif __name__ == "__main__":\n    main()`;
    }
    return `// Your code here\n// Complete the task described above\n\nfunction main() {\n  // TODO: Implement your solution\n}\n\nmain();`;
  ...
  // Create mini projects for Python e-commerce
  for (let i = 1; i <= 5; i++) {
    await dbStorage.createMiniProject({
      bigProjectId: pythonProject.id,
      miniId: i,
      title: `Mini Project ${i}: ${getMiniProjectTitle(i)}`,
      concepts: getMiniProjectConcepts(i),
      theory: getMiniProjectTheory(i),
      examples: getMiniProjectExamples(i, "python"),
      taskDescription: getMiniProjectTask(),
      hint: getMiniProjectHint(),
      codeTemplate: getCodeTemplate(i, "python"),
      expectedOutput: "Success! Task completed.",
      dependencies: i > 1 ? [`${i - 1}`] : [],
    });
  }
  ...
  for (let i = 1; i <= 5; i++) {
    await dbStorage.createMiniProject({
      ...
      concepts: getMiniProjectConcepts(i),
      theory: getMiniProjectTheory(i),
      examples: getMiniProjectExamples(i, "javascript"),
      taskDescription: getMiniProjectTask(),
      hint: getMiniProjectHint(),
      codeTemplate: getCodeTemplate(i, "javascript"),
      expectedOutput: "Console output: Success!",
      dependencies: i > 1 ? [`${i - 1}`] : [],
    });
  }
...
// Export MemStorage for backwards compatibility (if needed for testing)
